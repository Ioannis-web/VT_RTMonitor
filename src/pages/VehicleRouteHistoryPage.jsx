...
export default function VehicleRouteHistoryPage() {
  ...
  const loadVehicles = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("vehicles")
      .select("id, name, code, plate_number, vehicle_type, status")
  ...
  const loadRoute = useCallback(async () => {
    ...
    if (error) {
      ...
      alert("Σφάλμα φόρτωσης διαδρομής");
      setRoutePoints([]);
      ...
      <div className="p-6 space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">
              Ιστορικό Διαδρομής Οχήματος
            </h1>
            <p className="text-slate-600">
              Προβολή ιστορικών GPS στιγμάτων και διαδρομής πάνω στον Google Map.
            </p>
          </div>

          <button
            onClick={loadRoute}
            disabled={!selectedVehicleId || loadingRoute}
            className="bg-emerald-700 text-white px-4 py-2 rounded-lg hover:bg-emerald-800 disabled:opacity-50"
          >
            {loadingRoute ? "Φόρτωση..." : "Ανανέωση διαδρομής"}
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow p-5 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            {loadingVehicles ? (
              <div className="text-slate-500">Φόρτωση οχημάτων...</div>
            ) : (
              <select
          ...
          <div className="bg-slate-50 rounded-xl p-3 text-sm">
            <div className="font-semibold mb-1">Επιλεγμένο όχημα</div>
            <div>{selectedVehicle?.name || "-"}</div>
            <div className="text-slate-500">
              {selectedVehicle?.code || "-"} / {selectedVehicle?.status || "-"}
            </div>
          </div>
        </div>
          ...
          <div className="xl:col-span-3 bg-white rounded-2xl shadow overflow-hidden">
            <div className="p-4 border-b flex items-center justify-between">
              <div className="font-semibold">
                Route Map — {routePoints.length} σημεία
              </div>
                ...
                {selectedPoint && (
                  ...
                  >
                    <div className="text-sm space-y-1 min-w-[220px]">
                      <div className="font-bold">Στίγμα οχήματος</div>
          ...
          <div className="bg-white rounded-2xl shadow overflow-hidden">
            ...
            {loadingRoute ? (
              ...
              <div className="p-4 text-slate-500">
                Δεν υπάρχουν στίγματα για αυτή την ημερομηνία.
              </div>
            ) : (
